...
/**
 * Generated class for the AdminBookingListPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-admin-booking-list',
  templateUrl: 'admin-booking-list.html',
})
export class AdminBookingListPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad AdminBookingListPage');
  }
  dateChanged() {
        this.getBookingDetails();
    }
   ...
   getBookingDetails(){
        this.showLoader();
            this.authService.getBookingDetails(moment(this.selectedDate).format('YYYY-MM-DD'), moment(this.selectedDate1).format('YYYY-MM-DD')).then((result) => {
    ...
    showNewBookRequest(){
     var DocumentPagemodalPage = this.modalCtrl.create('BookingPage');
