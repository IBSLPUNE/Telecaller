...
/**
 * Generated class for the SchemesListPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-schemes-list',
  templateUrl: 'schemes-list.html',
})
export class SchemesListPage {
 ...
 constructor(public navCtrl: NavController, public navParams: NavParams, private authService: AuthServiceProvider, public loadingCtrl: LoadingController, private toastCtrl: ToastController, private modalCtrl: ModalController, public alertCtrl: AlertController, public popoverCtrl: PopoverController) {
  ...
  showNewSchemeRequest(){
   ...
   getAllSchemeList(){
        this.showLoader();
            this.authService.getAllSchemeList(moment(this.selectedDate).format('YYYY-MM-DD'), moment(this.selectedDate1).format('YYYY-MM-DD')).then((result) => {
